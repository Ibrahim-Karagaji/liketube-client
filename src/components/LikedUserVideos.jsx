...
export default function LikedVideos() {
  ...
  async function handleDeleteLikeFromVideo(videoid) {
    setLoading(true);

    try {
      const result = await deleteLikeFromVideoService(videoid);
  ...
  return (
    <div className={clsx("rounded-[5px] h-133 md:h-130 overflow-auto")}>
      ...
      >
        <i className="fa-solid fa-thumbs-up text-center md:text-[25px] text-[20px]"></i>
        <h2 className="text-center mt-3 font-semibold tracking-tight">
          {settings.settingsState.language == "English"
            ? "liked videos"
            : "مقاطع الفيديو المفضلة"}
        </h2>
        <p className="text-center text-[11px] sm:text-[13px] text-gray-300">
          {settings.settingsState.language == "English"
            ? "here's list of videos that you liked"
            : "إليك قائمة بالفيديوهات التي أعجبتك"}
        </p>
        {likedVideos.likeVideosState.videos.length !== 0 ? (
          <div
            className={clsx(
              "grid gap-2 p-2 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 rounded-[5px]"
            )}
          >
            {likedVideos.likeVideosState.videos.map((item, index) => (
              <div className="grid">
                <div
                  onClick={() => {
                ...
                <button
                  onClick={() => {
                    handleDeleteLikeFromVideo(item.id);
                  }}
                  className={clsx(
